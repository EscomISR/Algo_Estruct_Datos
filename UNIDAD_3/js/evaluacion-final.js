(() => {
  ...
  function validateData(loadedCase, loadedQuestions) {
    if (
      !loadedCase ||
      !Array.isArray(loadedCase.sections) ||
      !Array.isArray(loadedCase.resources)
    ) {
      throw new Error(
        "El archivo del caso no contiene las secciones o recursos esperados.",
      );
    }
    ...
    if (ids.size !== 28)
      throw new Error("Los identificadores de las preguntas deben ser únicos.");

    Object.entries(attemptCounts).forEach(([type, minimum]) => {
      const total = loadedQuestions.filter(
        (question) => question.type === type,
      ).length;

      if (total < minimum) {
        throw new Error(
          `El tipo ${type} debe contener al menos ${minimum} preguntas.`,
        );
      }
    });

    const resourceIds = new Set(
      loadedCase.resources.map((resource) => resource.id),
    );
    loadedQuestions
  ...
  async function loadEvaluation() {
    ...
    try {
      ...
      if (!caseResponse.ok || !questionsResponse.ok)
        throw new Error("No se pudieron leer los archivos JSON.");
      ...
      $("loadErrorMessage").textContent =
        error.message || "Verifica los archivos de datos e intenta nuevamente.";
    }
  }
  ...
  function resourceMarkup(resource, compact = false) {
    ...
    if (resource.type === "table") {
      const headings = resource.columns
    ...
    const bars = resource.bars
      .map((bar) => {
        ...
        <div class="bar-track" aria-hidden="true">
          <div class="bar-column" style="--bar-height:${height}%"></div>
        </div>
  ...
  function renderCaseContent(target, prefix) {
    ...
    const designSections = [
      ...
      .map((id) => sectionMarkup(section(id)))
      .join("");
    const resultResources = caseData.resources
      .map(
        (item) =>
          `<div class="${item.type === "table" ? "result-wide" : "result-charts"}">${resourceMarkup(item)}</div>`,
      )
      .join("");
  ...
  function prepareAttempt() {
    ...
    currentIndex = 0;
    const selectedQuestions = Object.entries(attemptCounts).flatMap(
      ([type, total]) =>
        shuffle(
          baseQuestions.filter((question) => question.type === type),
        ).slice(0, total),
    );
    questions = shuffle(selectedQuestions).map((question) => {
      const prepared = { ...question };
      if (Array.isArray(question.options))
        prepared.displayOptions = shuffle(question.options);
      if (question.type === "clasificar")
        prepared.displayItems = shuffle(question.items);
  ...
  function startEvaluation() {
    ...
    document
      .querySelector(".question-progress")
      ?.setAttribute("aria-valuemax", String(questions.length));
    startTimer();
    renderQuestion();
  ...
  function questionHeading(question) {
    return escapeHtml(
      question.question || question.instruction || "Clasifica los elementos.",
    );
  }

  function singleChoiceMarkup(question) {
    const selected = answers.get(question.id) || "";
    return `<div class="options-list">${question.displayOptions
      .map((option, index) => {
  ...
  function classificationMarkup(question) {
    const selected = answers.get(question.id) || {};
    const options = question.categories
      .map(
        (category) =>
          `<option value="${escapeHtml(category)}">${escapeHtml(category)}</option>`,
      )
      .join("");
    return `<div class="classification-list">${question.displayItems
      .map(
        (item) => `
  ...
  function orderingMarkup(question) {
    ...
    return `<div class="ordering-list">${segments
      .map(
        (segment, index) => `
  ...
  function renderQuestion() {
    ...
    const scenario = question.scenario
      ? `<div class="scenario-box"><strong>Situación:</strong> ${escapeHtml(question.scenario)}</div>`
      : "";
    const resource = question.resourceId
      ? `<div class="question-resource">${resourceMarkup(
          caseData.resources.find((item) => item.id === question.resourceId),
          true,
        )}</div>`
      : "";
    ...
    let interaction = "";
    if (
      ["opcion_multiple", "interpretar_datos", "identificar_error"].includes(
        question.type,
      )
    )
      interaction = singleChoiceMarkup(question);
    if (question.type === "seleccion_multiple")
      interaction = multipleChoiceMarkup(question);
    if (question.type === "clasificar")
      interaction = classificationMarkup(question);
    if (question.type === "ordenar_proceso")
      interaction = orderingMarkup(question);
  ...
  function isAnswered(question) {
    ...
    if (question.type === "clasificar")
      return question.items.every((item) => Boolean(answer?.[item.text]));
    if (question.type === "ordenar_proceso")
      return touchedOrders.has(question.id);
    return false;
  }
  ...
  function updateNavigation() {
    ...
    $("questionDots").innerHTML = questions
      .map(
        (question, index) =>
          `<button type="button" class="question-dot${index === currentIndex ? " is-current" : ""}${isAnswered(question) ? " is-answered" : ""}" data-question-index="${index}" aria-label="Ir a la pregunta ${index + 1}" ${index === currentIndex ? 'aria-current="step"' : ""}>${index + 1}</button>`,
      )
      .join("");
  }
  ...
  function goToQuestion(index) {
    ...
    window.scrollTo({
      top: Math.max(0, questionCard.offsetTop - 90),
      behavior: "smooth",
    });
  }
  ...
  function handleQuestionChange(event) {
    ...
    if (event.target.matches('input[type="checkbox"]')) {
      const selected = [
        ...questionCard.querySelectorAll('input[type="checkbox"]:checked'),
    ...
    if (event.target.matches(".classification-select")) {
      ...
      if (event.target.value)
        selected[event.target.dataset.item] = event.target.value;
  ...
  function handleQuestionClick(event) {
    ...
    const targetIndex =
      button.dataset.orderAction === "up" ? index - 1 : index + 1;
  ...
  function openPendingDialog() {
    ...
    lastDialogFocus = document.activeElement;
    $("pendingDialogText").textContent =
      `Faltan ${pending} ${pending === 1 ? "pregunta" : "preguntas"}. Si entregas ahora, se contarán como incorrectas.`;
  ...
  function isCorrect(question) {
    ...
    if (question.type === "seleccion_multiple")
      return arraysEqualAsSets(answer, question.answer);
    if (question.type === "clasificar")
      return question.items.every(
        (item) => answer[item.text] === item.category,
      );
    if (question.type === "ordenar_proceso")
      return answer.every(
        (segment, index) => segment === question.answer[index],
      );
    return false;
  }
  ...
  function formatAnswer(question, answer, correct = false) {
    ...
    if (question.type === "clasificar") {
      const source = correct
        ? Object.fromEntries(
            question.items.map((item) => [item.text, item.category]),
          )
        : answer;
      return question.items
        .map(
          (item) => `${item.text} → ${source?.[item.text] || "Sin clasificar"}`,
        )
        .join(" | ");
    }
  ...
  function submitEvaluation(timeExpired = false) {
    ...
    $("categoryPerformance").innerHTML = performanceCategories
      .map((category) => {
        const categoryQuestions = questions.filter(
          (question) => question.category === category,
        );
        const categoryCorrect = categoryQuestions.filter(isCorrect).length;
        const categoryPercentage = categoryQuestions.length
    ...
    $("questionReview").innerHTML = questions
      .map((question, index) => {
  ...
  function handleGlobalKeydown(event) {
    ...
    if (!$("pendingDialog").classList.contains("is-hidden"))
      closePendingDialog();
    else if (!$("caseDrawerOverlay").classList.contains("is-hidden"))
      closeCaseDrawer();
  ...
  document.querySelector(".drawer-tabs")?.addEventListener("click", (event) => {
    ...
    if (!button) return;
    document
      .getElementById(button.dataset.caseTarget)
